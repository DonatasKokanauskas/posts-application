...
export default {
    ...
    methods: {
        ...mapActions(["editAuthor", "closeModalAction"]),
        async submitForm() {
            this.errors = [];
            ...
            if (
                ...
                const editedAuthor = {
                    id: this.modalDataGetter.id,
                    name: this.name.replace(/\s+/g, " ").trim(),
                    created_at: this.modalDataGetter.created_at,
                    updated_at: new Date().toLocaleString("lt-LT").slice(0, 11),
                };

                await this.editAuthor(editedAuthor);
