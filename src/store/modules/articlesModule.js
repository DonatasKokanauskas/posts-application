...
const articlesModule = {
  ...
  actions: {
    ...
    async deleteArticleAction(
      ...
      try {
        await this.deleteData(`${rootState.apiURL}/posts/${articleIdToDelete}`);

        if (getters.allArticles.length - 1 === 0 && getters.currentPage > 1) {
    ...
    async editArticle({ rootState, getters, dispatch, commit }, editedArticle) {
      try {
        await this.editData(
